...
// Delete Member ByID locates the album whose ID value matches the id
// parameter sent by the client, then returns that corresponding message.
func (env Env) DeleteMemberByID(c *gin.Context) {
	...
	q := `DELETE FROM Member WHERE id = $1;`
	result, err := env.DB.Exec(q, id)
	if err != nil {
		e := fmt.Sprintf("error occurred while deleting member record with id: %d and error is: %v", id, err)
		log.Println(e)
	...
	// records to delete for this given member ID.
	if n == 0 {
